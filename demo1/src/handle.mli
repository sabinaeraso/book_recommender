...
val _handle_yes : state:Book_recommender.State.t -> unit
...
val _handle_no : state:Book_recommender.State.t -> unit
...
val _handle_read_yes : state:Book_recommender.State.t -> unit

(* They are not interested. Add the current book to visited and update the
   state with these new lists. Generate a new book and print it out. *)
val _handle_read_no : state:Book_recommender.State.t -> unit

(* They dont want to see any more books. Print out their recommended books.*)
val _handle_done : state:Book_recommender.State.t -> unit
